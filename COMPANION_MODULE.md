    ...
    **Comportement** :
    - **Pour `focus` uniquement** : Le delta est ajouté à la valeur actuelle (normalisée), puis la valeur est clampée entre 0.0 et 1.0
    - **Note** : `nudge` ne fonctionne que pour `focus` (valeur continue). Pour `iris`, `gain`, `shutter` et `whiteBalance` (valeurs discrètes), utilisez la commande `adjust_param` avec `direction: "up"` ou `"down"`.
    ...
    **Comportement** :
    - **Pour `iris`** : Incrémente ou décrémente l'iris d'un stop d'aperture (utilise `adjustmentStep` de l'API Blackmagic). Chaque appel ajuste d'un stop complet (ex: f/2.8 → f/4.0 ou f/4.0 → f/2.8)
